...
dirNum = 50
# fileNumber 文件的序号
R_row = 40 # block size row
C_col = 40 # block size column
for fileNumber in range(31, dirNum+1):
    ...
    layer_Row = sizeRow // R_row  # 越大越矮
    layer_Column = sizeColumn // C_col  # 越大越细
    ...
    for i in range(0, layer_Row):
        for j in range(0, layer_Column - 1):
            if result_vis[i][j] == result_vis[i][j + 1]:
                ...
                dislocation[i][j] = totalScore / (C_col * R_row) * 100
        ...
        for j in range(0, layer_Column - 3):
            if (dislocation[i][j] >= 1):
                for k in range(st+1, ed+1):
                    ...
                    score2 = similar_Frank(ImageBlock_2, ImageBlock_4, True)
                    # 计算替换块，左边的相邻图像块的错位率
                    disRate_Left = (score1 + score2) / (C_col * R_row) * 100
                    ...
                    # 计算替换块，左边的相邻图像块的错位率
                    disRate_Right = (score3 + score4) / (C_col * R_row) * 100
    ...
    imwrite(projectPath + 'FAPImages/algorithm_10/' + str(fileNumber) + '_0' + '.bmp', img0)
    print("block40*40 " + str(fileNumber) + "finish！")