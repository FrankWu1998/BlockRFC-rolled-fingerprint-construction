...
dirNum = 7
# fileNumber 文件的序号
for fileNumber in range(7, dirNum+1):
    ...
    imgRGB = cv2.cvtColor(img0, cv2.COLOR_GRAY2BGR)
    # 以时间戳保存拼接完成的图像
    for i in range(0, layer_Row):
        for j in range(0, layer_Column):
            if (dislocation[i][j] < 1):
                ...
                TagDislocation(i, j, imgRGB)

    nowTime = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')  # 现在
    imwrite(projectPath + 'ExpRecord/' + nowTime + '(优化前)' + '.jpg', imgRGB)
    ...
    # 主次目标优化
    for i in range(0, layer_Row):
        for j in range(0, layer_Column - 2):
            if (dislocation[i][j] >= 1):
                for k in range(st + 1, ed + 1):
    ...
    imgRGB_Optimization = cv2.cvtColor(img0, cv2.COLOR_GRAY2BGR)
    ...
    for i in range(0, layer_Row):
        for j in range(0, layer_Column):
            if (dislocation[i][j] < 1):
                ...
                TagDislocation(i, j, imgRGB_Optimization)

    # cv2.imshow("Ans", imgRGB_Optimization)
    # cv2.waitKey(10000)
    imwrite(projectPath + 'ExpRecord/' + nowTime + '(优化后)' + '.jpg', imgRGB_Optimization)
        ...
        for j in range(0, layer_Column):
            ...
            if (dislocation[i][j] < 1):
                ...
                if (whiteRate > 60 or mean1 > 200 or mean2 > 200):
                    ...
                    disBad[i][j] = 1
                    TagDislocation(i, j, tempImg)

    imwrite(projectPath + 'ExpRecord/' + nowTime + '(优化并去边缘化)' + '.jpg', tempImg)